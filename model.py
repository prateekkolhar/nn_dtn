    ...
    def content_extractor(self, images, reuse=False):
        ...
        with tf.variable_scope('content_extractor', reuse=reuse):
            with slim.arg_scope([slim.conv2d], padding='SAME', activation_fn=tf.nn.relu,
                ...
                net = slim.max_pool2d(net, [2,2], stride=[2,2], padding='SAME', scope='bn1')
                # (batch_size, 16, 16, 32)
                net = slim.conv2d(net, 32, [3, 3], scope='conv2')
                net = slim.max_pool2d(net, [2,2], stride=[2,2], padding='SAME', scope='bn2')
                # (batch_size, 8, 8, 32)
                net = slim.conv2d(net, 16, [3, 3], scope='conv3')
                net = slim.max_pool2d(net, [2,2], stride=[2,2], padding='SAME', scope='bn3')
                # (batch_size, 4, 4, 16)
